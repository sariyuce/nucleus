...
inline lol NEWnChoosek(lol n, int k) {
	if (k > n)
		return 0;
	if (k * 2 > n)
		k = n - k;
...
void tipDecomposition (Graph& leftGraph, Graph& rightGraph, edge nEdge, vector<vertex>& K, bool hierarchy, lol* maxbicore, string vfile, FILE* fp, lol* bCount) {
	...
	cout << "Counting butterflies per vertex time: " << c2 - c1 << endl;
	...
	while (true) {
		...
		for (auto it = dup.begin(); it != dup.end(); it++) {
			...
			if (x != u && count > 1) {
				if (K[x] == -1) {
					lol decreaseAmount = NEWnChoosek (count, 2);
	...
	*maxbicore = bf_u;
	...
	if (!hierarchy) {
		...
		cout << "Total time: " << (p2 - p1) + (c2 - c1) << endl;
	}
	else {
		...
		cout << "Total time (excluding density computation): " << (p2 - p1) + (c2 - c1) + (b2 - b1) << endl;
		...
		cout << "Total time: " << (p2 - p1) + (c2 - c1) + (b2 - b1) + (d2 - d1) << endl;
	}
