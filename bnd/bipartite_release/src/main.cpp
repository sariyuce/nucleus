...
int main(int argc, char *argv[]) {
	...
	if (nd == "RIGHT_TIP")
		...
		if (leftGraph.size() < rightGraph.size()) { // select the smaller set for faster computation
			...
			if (hierarchy)
				...
				wingDecomposition (rightGraph, leftGraph, nEdge, K, hierarchy, &maxK, vfile, fp, &bCount);
		}
		else {
			...
			if (hierarchy)
				...
				wingDecomposition (leftGraph, rightGraph, nEdge, K, hierarchy, &maxK, vfile, fp, &bCount);
		}
	}
		...
		base_ktruss (gr, hierarchy, nEdge, K, hierarchy, &maxK, vfile, fp); // run k-truss on the given projection graph
	else if (nd == "RUN_WEIGHTED_CORE")
		weighted_base_kcore (wg, nEdge, K, hierarchy, &maxK, vfile.c_str(), fp); // run fractional core on the given weighted projection graph
	...
	if (hierarchy)
		cout << "Total time (peeling + hierarchy): " << t2 - t1 << endl;
	else
		cout << "Total time (peeling): " << t2 - t1 << endl;
