...
vertex count_outps (Graph& dgraph, vector<vertex>& TC) {
	vertex count = 0;
	for (vertex u = 0; u < dgraph.size(); u++) {
		vector<vertex> ret;
		asymmetric_undirecteds (u, dgraph[u], ret); // u-v edge is undirected
...
void outp_core (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* maxK, FILE* fp) {
	...
	// each node and its outp count is inserted to bucket
	for (vertex i = 0; i < graph.size(); i++) {
		printf ("outp count of %d is %d\n", i, TC[i]);
		if (TC[i] > 0)
	...
	while (true) {
		...
		for (vertex r = 0; r < ret.size(); r++) {
			...
			if (K[v] == -1) { // v might not be a part of outp, but must have -1 kappa if so anyways
				...
				for (auto k = 0; k < ints.size(); k+=2) {
					vertex w = M2P (graph[u][ints[k]]); // triangle node (and equal to M2P (graph[v][ints[k+1]]))
		...
		for (vertex r = 0; r < ret.size(); r++) {
			vertex v = graph[u][ret[r]]; // circle node orbit
			if (K[v] == -1) { // v might not be a part of outp, but must have -1 kappa if so anyways
				vector<vertex> ints;
				inter (2, 0, graph, u, v, ints);
				for (auto k = 0; k < ints.size(); k+=2) {
					vertex w = graph[u][ints[k]]; // circle node
...
//					checkAndDec (K[w], -1, w, v, &nBucket, tc_u);
	...
	*maxK = tc_u;
	...
	if (!hierarchy) {
		...
		print_time (fp, "Total outp-core nucleus decomposition time (excluding density computation): ", (p2 - p1) + (t2 - t1) + (b2 - b1));
		...
		print_time (fp, "Total outp-core nucleus decomposition time: ", (p2 - p1) + (t2 - t1) + (b2 - b1) + (d2 - d1));
	}
