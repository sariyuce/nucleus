...
// per edge
lol countTriangles (Graph& graph, Graph& orderedGraph, Graph& TC, vector<int>& gender, int labelType) {
...
void base_ktruss (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* maxtruss, string vfile, FILE* fp, vector<int>& gender, int labelType) {
...
#ifndef LOAD_TRIS
	// Compute
	tric = countTriangles (graph, orderedGraph, TC, gender, labelType);
#else
	// Load
	...
	for (size_t i = 0; i < orderedGraph.size(); i++)
		for (size_t j = 0; j < orderedGraph[i].size(); j++) {
			if (TC[i][j] > 0) {
				vertex a = el[xel[i]+j].first;
				vertex b = el[xel[i]+j].second;
//				printf("%d %d: %d\n", gender[a], gender[b], TC[i][j]);
...
//				vertex a = el[xel[i]+j].first;
//				vertex b = el[xel[i]+j].second;
//				printf("%d %d    %d %d : %d %d\n", a, b, gender[a], gender[b], TC[i][j], K[xel[i]+j]);
////			}
//		}
