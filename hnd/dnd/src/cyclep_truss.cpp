	...
	for (vertex i = 0; i < graph.size(); i++) {
		...
		for (vertex r = 0; r < ret.size(); r++) {
			...
			if (ret[r] < 0)
				...
				c = make_pair (i, v); // directed
			el.push_back(c);
//			printf ("cyclep count of %d (%d-%d) is %d\n", id, i, v, TC[i][ind]);
			if (TC[i][ind] > 0) {
				nBucket.Insert (id++, TC[i][ind]);
				dist[TC[i][ind]]++;
			}
...
//				checkAndDec (K[id1], K[id2], id1, id2, &nBucket, tc_e);
	...
	*maxK = tc_e;
	...
	if (!hierarchy) {
		...
		print_time (fp, "Total cyclep-truss nucleus decomposition time (excluding density computation): ", (p2 - p1) + (t2 - t1) + (b2 - b1));
		...
		print_time (fp, "Total cyclep-truss nucleus decomposition time: ", (p2 - p1) + (t2 - t1) + (b2 - b1) + (d2 - d1));
	}
