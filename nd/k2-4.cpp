...
// per edge
void count4cliques (Graph& graph, Graph& orientedGraph, vector<vertex>& xel, vector<vp>& el, vector<vertex>& FC) {
...
void base_k24 (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vertex* max24, string vfile, FILE* fp) {
	...
	count4cliques (graph, orientedGraph, xel, el, FC);
	...
	nBucket.Initialize(nEdge, nEdge);
