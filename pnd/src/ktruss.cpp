...
//		printf ("order %d %d\n", order++, uv);
	...
	for (int i = 0; i < el.size(); i++)
		fprintf (pp, "%d %d   %d\n", get<0>(el[i]), get<1>(el[i]), (T[i] == -1 ? 0 : T[i]));
	fclose (pp);
	return;
}
...
void ktruss_levels (Graph& graph, int nEdge, vector<vertex>& T, util::timestamp& totaltime, int* truss_number, const char* vfile, FILE* fp) {
	...
	int level = 1;
	while (1) {
		int uvo, value_of_uvo;
		int ret;

		int val;
		vector<int> mins;
		ret = na_bs.PopMin(&uvo, &value_of_uvo);
		if (ret == -1) // if the bucket is empty
			break;
		...
		mins.push_back (uvo);
		while (1) {
			ret = na_bs.PopMin(&uvo, &val);
			if (ret == -1) // if the bucket is empty
				break;
			if (val > value_of_uvo) {
				na_bs.Insert (uvo, val);
				break;
			}
			mins.push_back (uvo);
		}
		...
		for (int i = 0; i < mins.size(); i++) {
			...
			for (size_t j = 0; j < intersection.size(); j++) { /* decrease the tc of the neighbor edges with greater tc */
				...
				if (T[id1] < 0 && T[id2] < 0) {
					...
					if (cur_id1 > value_of_uvo)
						na_bs.DecVal(id1);

					if (cur_id2 > value_of_uvo)
						na_bs.DecVal(id2);
				}
			}
		}
		printf ("Level %d K: %d size: %d\n", level, value_of_uvo, mins.size());
		level++;
